class Education {
  constructor() {
    ...
    this.background_text.textContent = "EDUCATION";
    this.root.appendChild(this.background_text);

    const textContent = "Education".split("");
    ...
    this.all_education_holder = document.createElement("div");
    this.all_education_holder.id = "all-education-holder";
    this.all_education_holder.classList.add("all-holder");

    this.education_card_animations = new Array(__EDUCATION.length).fill(null);

    this.cards = [];
    __EDUCATION.forEach((edu, i) => {
      const education_card = document.createElement("div");
      education_card.id = "education-" + i;
      education_card.classList.add("card");

      const title = document.createElement("h3");
      title.id = "education-title-" + i;
      title.textContent = edu.title;
      const school = document.createElement("h4");
      school.id = "education-school-" + i;
      school.textContent = edu.school;
      const date = document.createElement("p");
      date.id = "education-date-" + i;
      ...
      edu.skills.forEach((skill, j) => {
        const skill_element = document.createElement("div");
        skill_element.id = "education-skill-" + i + "-" + j;
      ...
      education_card.appendChild(tassel);
      education_card.appendChild(title);
      education_card.appendChild(school);
      education_card.appendChild(date);
      education_card.appendChild(skills_holder);
      education_card.style.opacity = this.mobileCheck() ? 1 : 0;

      this.cards.push(education_card);
      this.all_education_holder.appendChild(education_card);
    });

    this.root.appendChild(this.all_education_holder);
  }
    ...
    const observer = new IntersectionObserver(
      (entries, observer) => {
        ...
        if (!this.experience_loaded) {
          anime({
            targets: "#education-section-title span",
            opacity: [0, 1],
            marginLeft: [10, 0],
