...
// import { OBJLoader } from './module/OBJLoader.js';
import {
  AnimationClip,
  Bone,
  Box3,
...
const SMOOTH_SCROLL_FACTOR = 0.9;
const SCROLL_SENSITIVITY = 0.00015;
const ANIMATION_SPEED = 1.2;
const MOUSE_MOVE_SENSITIVITY = 0.0007;
const ANIMATION_TIME = 10.416666;
const MAXIMUM_ROTATION = Math.PI * 2;
const FINAL_ROTATION = 6.9566;
var FOG_COLOR = 0x598759;
var FOG_COLOR_TRANSISION = 0x38563a;
const MONTHS = [
  "AUG",
  "SEP",
  "OCT",
...
function init() {
  ...
  textureLoader.load(
    "./assets/Texture/STARS_HDRi.jpg" /* '/assets/hdri/HDR_Blurred.jpg' */,
    function (txtr) {
      // geometry
      var geometry = new THREE.SphereGeometry(300, 240, 240);
  ...
  //0x83EA83 , 0x0E638F , 0x598759
  scene.fog = new THREE.Fog(FOG_COLOR, 1, 700);
...
function onScroll(event) {
  // console.log(Math.round(10*CURRENT_POSITION/FINAL_ROTATION),currentMonth)
  if (Math.round((10 * CURRENT_POSITION) / FINAL_ROTATION) !== currentMonth) {
    lastMonth = currentMonth;
    currentMonth = Math.round((10 * CURRENT_POSITION) / FINAL_ROTATION);
  ...
  SCROLL_ALPHA = event.deltaY * SCROLL_SENSITIVITY;
  if (event.deltaY > 0) {
    // rotateMain(true)
    // MAIN_GROUP.rotation.y += 0.05
    // if (MIXER) MIXER.update( ANIMATION_SPEED );
    // if(camera.position.z > 90) camera.position.z -= 1;
  }
  if (event.deltaY < 0) {
    // rotateMain(false)
    // MAIN_GROUP.rotation.y -= 0.05
...
function rotateMain(direction) {
  if (direction === true) {
    var loop = setInterval(() => {
      if (PROGRESS_COUNT < 100) {
        PROGRESS_COUNT++;
        MAIN_GROUP.rotation.y += 0.005;
      } else {
        PROGRESS_COUNT = 0;
        clearInterval(loop);
      }
...
function onMouseMove(e) {
  ...
  X_MOVEMENT = e.clientX - window.innerWidth / 2;
  // Y_MOVEMENT = e.clientY - window.innerHeight/2;

  // camera.position.x += X_MOVEMENT*MOUSE_MOVE_SENSITIVITY;
  // MAIN_GROUP.rotation.y = Y_MOVEMENT*MOUSE_MOVE_SENSITIVITY;
}
...
function render() {
  ...
  // MAIN_GROUP.rotation.y = Y_MOVEMENT;
  if (EXPLORE_MODE) {
    if (CURRENT_POSITION < FINAL_ROTATION) {
      CURRENT_POSITION += SCROLL_ALPHA;
      SCROLL_ALPHA = SCROLL_ALPHA * SMOOTH_SCROLL_FACTOR;
      MAIN_GROUP.rotation.y = CURRENT_POSITION;
      if (MIXER) {
        ANIMATION_PROGRESS += SCROLL_ALPHA;
        if (ANIMATION_PROGRESS > 0)
          MIXER.update(SCROLL_ALPHA * ANIMATION_SPEED);
        // MIXER.update( ANIMATION_SPEED*SCROLL_ALPHA)
      }
    } else {
      if (SCROLL_ALPHA < 0) {
        CURRENT_POSITION += SCROLL_ALPHA;
    ...
    MAIN_GROUP.rotation.y =
      0.2 + X_MOVEMENT * MOUSE_MOVE_SENSITIVITY + CURRENT_POSITION;
  } else {
    MAIN_GROUP.rotation.y = 0.2 + X_MOVEMENT * MOUSE_MOVE_SENSITIVITY;
  }
  // ANIMATION_TIME/MAXIMUM_ROTATION
  // CURRENT_POSITION/MAXIMUM_ROTATION
  // console.log(ANIMATION_PROGRESS)
  camera.lookAt(scene.position);
  renderer.render(scene, camera);
}

// var timelineBTN = document.getElementById("Timeline_btn");
// timelineBTN.addEventListener("click", enterTimeline);
function enterTimeline() {
  $("#Main_Page").fadeOut(400, () => {
    var cnt = 0;
    var loop = setInterval(() => {
      cnt++;
      // console.log(10-cnt/10)
      // MIXER.update(-0.1)
      // if(cnt < 50) scene.fog = new THREE.Fog(FOG_COLOR_TRANSISION,1,700-cnt*2*7);
      if (cnt == 50) MIXER.setTime(0);
      // if(cnt >= 50) scene.fog = new THREE.Fog(FOG_COLOR_TRANSISION,1,0+cnt*2*7);

      if (cnt >= 100) {
        clearInterval(loop);
        cnt = 0;
        // scene.fog = new THREE.Fog(FOG_COLOR_TRANSISION,1,700);
      }
    }, 1000 / 100);
    CURRENT_POSITION = MAIN_GROUP.rotation.y;

    $("#TimelinePage").fadeIn(400, () => {
      EXPLORE_MODE = true;
    });
  });
}
